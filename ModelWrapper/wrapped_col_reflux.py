...
def pre_setting(model):
    ...
    model.AirFeedTray = Param(initialize=1)
    model.RefluxTray = Param(initialize=model.NumberOfTrays.value)
    ...
    model.trayPressure = Var(model.AllTrays,bounds=(0,1000), initialize = (model.ColumnTopPressure.value+model.ColumnBottomPressure.value)/2)
...
def post_setting(model):
    ...
    #------------------Reflux-----------
    def RefluxRatio(m):
        return m.RefluxRatio * m.vaporLeavingMolarFlow[m.NumberOfTrays.value]
    ...
    def RefluxEnthalpy(m):
        return LiquidPhaseEnthalpy(m.RefluxTemperature, m.RefluxPressure,\
                                   m.vaporLeavingMolarFraction[m.NumberOfTrays.value,'Oxygen'],m.vaporLeavingMolarFraction[m.NumberOfTrays.value,'Nitrogen'])
    ...
    def materialBalances(m,tray, comp):
        if tray == m.NumberOfTrays.value:
            if tray == m.RefluxTray.value:
                return (m.vaporLeavingMolarFlowCompo[tray - 1, comp] \
                       ...
                       + m.ircRefluxMolarFlow * m.vaporLeavingMolarFraction[m.NumberOfTrays.value, comp])*1e-3 == 0
            else:
                return (m.vaporLeavingMolarFlowCompo[tray - 1, comp] \
                       - m.liquidLeavingMolarFlowCompo[tray, comp] - m.vaporLeavingMolarFlowCompo[tray, comp])*1e-3 == 0
        elif tray == 1:
            if tray == m.AirFeedTray.value:
    ...
    def ColumnPressureProfile(m, tray):
        return m.trayPressure[tray] == (m.ColumnTopPressure.value-m.ColumnBottomPressure.value)/(m.NumberOfTrays.value-1)*(tray-1)+m.ColumnBottomPressure.value
    ...
    # model.vaporLeavingMolarFraction = Var(model.AllTrays, model.Component, bounds=(0, 1), initialize=0.5)
    # model.liquidLeavingMolarFraction = Var(model.AllTrays, model.Component, bounds=(0, 1), initialize=0.5)
    def ITopVaporMolarFlow(m):
        return m.TopVaporMolarFlow == m.vaporLeavingMolarFlow[m.NumberOfTrays.value]
    model.ITopVaporMolarFlow = Constraint(rule=ITopVaporMolarFlow)
    def IBottomLiquidMolarFlow(m):
        return m.BottomLiquidMolarFlow == m.liquidLeavingMolarFlow[1]
    model.IBottomLiquidMolarFlow = Constraint(rule=IBottomLiquidMolarFlow)
    def ITopVaporNitrogenMolarFrac(m):
        return m.TopVaporNitrogenMolarFrac == m.vaporLeavingMolarFraction[m.NumberOfTrays.value, 'Nitrogen']
    model.ITopVaporNitrogenMolarFrac = Constraint(rule=ITopVaporNitrogenMolarFrac)
    def IBottomLiquidOxygenMolarFrac(m):
        return m.BottomLiquidOxygenMolarFrac == m.liquidLeavingMolarFraction[1, 'Oxygen']
    model.IBottomLiquidOxygenMolarFrac = Constraint(rule=IBottomLiquidOxygenMolarFrac)


setting = {}
setting['RefluxRatio'] = wm.SettingItem(init_value=0.6,bound=(0,1),domain=(0.5,0.8))
setting['RefluxTemperature'] = wm.SettingItem(init_value=-177,bound=(-230,-100),domain=(-190,-175))
setting['RefluxPressure'] = wm.SettingItem(init_value=539.56,bound=(300,600),domain=(520,550))
setting['TopVaporMolarFlow'] = wm.SettingItem(init_value=4315,bound=(0,10000),domain=(200,6000))
setting['BottomLiquidMolarFlow'] = wm.SettingItem(init_value=273,bound=(0,10000),domain=(50,2000))
setting['TopVaporNitrogenMolarFrac'] = wm.SettingItem(init_value=0.8567,bound=(0,1),domain=(0.8,0.99))
setting['BottomLiquidOxygenMolarFrac'] = wm.SettingItem(init_value=0.6303,bound=(0,1),domain=(0.2,0.9))
w_model = wm.WrappedConcreteModel(3, setting)
w_model.pre_setting = pre_setting
w_model.post_setting = post_setting
roc = w_model.get_range_of_converge(['RefluxRatio','RefluxTemperature','RefluxPressure'],n_div=3)
print(roc)
# A,b,infeasible_points = w_model.get_charasteristic(['GasDrawRatio','expandValveDeltaP'],n_sample=20)
# print(A)
# w_model.two_paras_analysis(['GasDrawRatio','expandValveDeltaP'],n_div=5)
# w_model.one_paras_analysis(['GasDrawRatio','expandValveDeltaP'],n_div=10)