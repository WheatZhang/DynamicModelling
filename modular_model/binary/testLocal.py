...
columnBodycompoThermo = ModularTask.Thermodynamics.CstmBinaryMargulesCompo(testTask)
...
testTask.loadNominalRange('NominalRange.txt')
...
options = {
    ...
    'CompositionThermo' : columnBodycompoThermo,
    'VaporEnthalpy':vap_enthalpy,
    'LiquidEnthalpy': liq_enthalpy,
    'PressureProfile' : 'SimpleLinear',
    'CreateCustimizedVLEFile' : True
}
...
testTask.generatePyomoModel("pyomoLocal.py")

from pyomoLocal import test_model
...
with open('local_display.txt', 'w') as file:
    test_model.display(ostream=file)
ModularTask.InitValueTools.to_template(test_model, 'StaticLocalInitValue.txt')