...
from binary_fitting_density import liquid_density
import tools
...
model.TrayCrossSectionArea=Param(initialize=6.5)
model.HeightOfWweir=Param(initialize=35e-3)
model.LengthOfWweir=Param(initialize=1.942)
#--------------进料指定----------------
FeedMolarFlow = 2000
FeedPressure = 564
...
def FeedInput(m,t):
    if t > 1:
        return FeedMolarFlow-200
    else:
        return FeedMolarFlow
model.FeedMolarFlow = Param(model.ParaTime, rule = FeedInput)
...
model.trayCompHoldupAmount = Var(model.time, model.AllTrays, model.Component, within = NonNegativeReals)
model.DtrayCompHoldupAmountDt = DerivativeVar(model.trayCompHoldupAmount)
model.DEnergyHoldupDt = Var(model.time, model.AllTrays)
model.DtrayPressureDt = DerivativeVar(model.trayPressure)
#-------------IRC变量定义---------------
...
# 认为各变量的函数依赖关系为： H_liquid = f(x,T,P)=x_oxygen*H_liq_Oxy(T,P)+x_nitrogen*H_liq_nitr(T,P)
...
#----------------------------------------
###
#   Model Index Reduction expression
###
#----------------------------------------
def trayHoldupEnthalpyPartialT(m, t,tray):
    return LiquidPhaseEnthalpyPartialT(m.trayTemperature[t,tray], m.trayPressure[t,tray], m.liquidLeavingMolarFraction[t,tray,'Oxygen'],\
                               m.liquidLeavingMolarFraction[t,tray,'Nitrogen'])
model.trayHoldupEnthalpyPartialT = Expression(model.time, model.AllTrays, rule=trayHoldupEnthalpyPartialT)
def trayHoldupEnthalpyPartialX(m, t,tray,c):
    if c == 'Oxygen':
        return liquid_oxygen_enthalpy(m.trayPressure[t,tray],m.trayTemperature[t,tray])
    elif c == 'Nitrogen':
        return liquid_nitrogen_enthalpy(m.trayPressure[t,tray], m.trayTemperature[t,tray])
model.trayHoldupEnthalpyPartialX = Expression(model.time, model.AllTrays,model.Component, rule=trayHoldupEnthalpyPartialX)
def DLiquidCompDt(m,t,tray,c):
    return (m.DtrayCompHoldupAmountDt[t,tray, c]-m.liquidLeavingMolarFraction[t,tray, c]*m.DtrayHoldupAmountDt[t,tray])/m.trayHoldupAmount[t,tray]
model.DLiquidCompDt = Expression(model.time, model.AllTrays,model.Component, rule=DLiquidCompDt)
def DTrayTempDt(m,t,tray):
    return (m.DLiquidCompDt[t,tray,'Oxygen']-liquid_oxygen_composition_DP(m.trayPressure[t,tray],m.trayTemperature[t,tray])*m.DtrayPressureDt[t,tray])/ \
           liquid_oxygen_composition_DT(m.trayPressure[t,tray],m.trayTemperature[t,tray])
model.DTrayTempDt = Expression(model.time, model.AllTrays, rule=DTrayTempDt)
#-----------------------------------
###
#   Model constraints
###
        ...
        if tray == RefluxTray:
            return (m.vaporLeavingMolarFlowCompo[t,tray - 1, comp] \
                   ...
                   + m.ircRefluxMolarFlow[t] * m.vaporLeavingMolarFraction[t,model.NumberOfTrays.value, comp] \
                    -m.DtrayCompHoldupAmountDt[t,tray,comp])*1e-3 == 0
        else:
            return (m.vaporLeavingMolarFlowCompo[t,tray - 1, comp] \
                   - m.liquidLeavingMolarFlowCompo[t,tray, comp] - m.vaporLeavingMolarFlowCompo[t,tray, comp] \
                    -m.DtrayCompHoldupAmountDt[t,tray,comp])*1e-3 == 0
        ...
        if tray == RefluxTray:
            return (m.vaporLeavingMolarFlow[t,tray - 1] * m.vaporLeavingMolarEnthalpy[t,tray - 1] \
                   ...
                   + m.ircRefluxMolarFlow[t] * m.ircCondLiquidMolarEnthalpy[t]\
                    -m.DEnergyHoldupDt[t,tray])*1e-5 == 0
        else:
            return (m.vaporLeavingMolarFlow[t,tray - 1] * m.vaporLeavingMolarEnthalpy[t,tray - 1] \
                   - m.liquidLeavingMolarFlow[t,tray] * m.liquidLeavingMolarEnthalpy[t,tray] \
                   - m.vaporLeavingMolarFlow[t,tray] * m.vaporLeavingMolarEnthalpy[t,tray]\
...
def ComponentHoldupEqu(m,t, tray, c):
    return (m.trayCompHoldupAmount[t,tray,c] - m.trayHoldupAmount[t,tray]* m.liquidLeavingMolarFraction[t,tray,c])*1e-2 == 0
model.ComponentHoldupEqu = Constraint(model.time, model.AllTrays,model.Component,rule = ComponentHoldupEqu)
...
#----------------Hydraulic-----------------
# return (m.trayHoldupAmount[t, tray] - m.hydraulicConstant * m.liquidLeavingMolarFlow[t, tray]) * 1e-3 == 0
def TrayHydraulicRelation(m, t, tray):
    return (m.TrayCrossSectionArea * liquid_density(m.trayPressure[t,tray], m.trayTemperature[t,tray]) * \
     (m.HeightOfWweir + 1.41 * (m.liquidLeavingMolarFlow[t,tray] / ((9.8*3600)**0.5 * liquid_density(m.trayPressure[t,tray], m.trayTemperature[t,tray])\
         * m.LengthOfWweir)) ** (2 / 3))-m.trayHoldupAmount[t,tray])*1e-2 == 0
model.TrayHydraulicRelation = Constraint(model.time, model.AllTrays, rule = TrayHydraulicRelation)
#---------------Energy Holdup Rewrite--------------
def TrayEnergyHoldupEqu(m,t,tray):
    return (m.trayHoldupAmount[t,tray]*(m.trayHoldupEnthalpyPartialT[t,tray]*m.DTrayTempDt[t,tray]+ \
            sum([m.trayHoldupEnthalpyPartialX[t,tray,c]*m.DLiquidCompDt[t, tray, c] for c in m.Component]))+ \
            m.liquidLeavingMolarEnthalpy[t, tray]*m.DtrayHoldupAmountDt[t,tray]-m.DEnergyHoldupDt[t,tray])*1e-2 == 0
model.TrayEnergyHoldupEqu = Constraint(model.time, model.AllTrays, rule = TrayEnergyHoldupEqu)
#-------------IRC Condenser Side-----------------
...
#-------------------------------------------
# 对于本模型，正确的初始条件指定方式是：M(0),dM/dt(0),m1(0),dH/dt(0)
def _trayHoldupAmountinitCond(m):
    yield m.trayHoldupAmount[0,1] ==  63.187777022926745
    yield m.trayHoldupAmount[0,2] ==   47.87126362494158
    yield m.trayHoldupAmount[0,3] ==   47.81225731380554
    yield m.trayHoldupAmount[0,4] ==   47.71405881285183
    yield m.trayHoldupAmount[0,5] ==    47.5617021510306
    yield m.trayHoldupAmount[0,6] ==   47.33563293738188
    yield m.trayHoldupAmount[0,7] ==   47.01215408812012
    yield m.trayHoldupAmount[0,8] ==   46.56596592145298
    yield m.trayHoldupAmount[0,9] ==   45.97607679184501
    yield m.trayHoldupAmount[0,10] == 45.235460220210506
model.trayHoldupAmountInitCond = ConstraintList(rule=_trayHoldupAmountinitCond)
trayCompHoldupAmountDict = {
(10, 1, 'Nitrogen') :    40.99236119026204,
  (10, 1, 'Oxygen') :    22.19541583266471,
(10, 2, 'Nitrogen') :   31.138231130953013,
  (10, 2, 'Oxygen') :   16.733032493988564,
(10, 3, 'Nitrogen') :    31.28932517746812,
  (10, 3, 'Oxygen') :    16.52293213633742,
(10, 4, 'Nitrogen') :   31.563212386519346,
  (10, 4, 'Oxygen') :   16.150846426332496,
(10, 5, 'Nitrogen') :    32.00723825528014,
  (10, 5, 'Oxygen') :   15.554463895750457,
(10, 6, 'Nitrogen') :   32.684501515227204,
  (10, 6, 'Oxygen') :   14.651131422154675,
(10, 7, 'Nitrogen') :    33.67379039364152,
  (10, 7, 'Oxygen') :   13.338363694478609,
(10, 8, 'Nitrogen') :    35.06343594440264,
  (10, 8, 'Oxygen') :   11.502529977050338,
(10, 9, 'Nitrogen') :    36.93426004927561,
  (10, 9, 'Oxygen') :    9.041816742569399,
(10, 10, 'Nitrogen') :    39.32836939639433,
 (10, 10, 'Oxygen') :    5.907090823816177
}
def trayCompHoldupAmntInitCond(m):
    for key,value in trayCompHoldupAmountDict.items():
        if key[2] == 'Nitrogen':
            yield m.trayCompHoldupAmount[0,key[1],key[2]] == value
model.trayCompHoldupAmntInitCond = ConstraintList(rule=trayCompHoldupAmntInitCond)
def DtrayCompHoldupAmntDtInitCond(m):
    for i in range(10):
        yield m.DtrayHoldupAmountDt[0,i+1] == 0
model.DtrayCompHoldupAmntDtInitCond = ConstraintList(rule=DtrayCompHoldupAmntDtInitCond)
def DEnergyHoldupDtInitCond(m):
    for i in range(10):
        yield m.DEnergyHoldupDt[0,i+1] == 0
model.DEnergyHoldupDtInitCond = ConstraintList(rule=DEnergyHoldupDtInitCond)
#-------------------------------------------
#       discretilization
#-------------------------------------------
...
#-------------------------------------------
load_initials.load_display_modified_t(model, "initials2.txt",pe_list,[0,0.310102,1.289898,2.0,2.310102,3.289898,4.0,4.310102,5.289898,6.0,6.310102,7.289898,8.0,8.310102,9.289898,10] )
