...
model.trayCompHoldupAmount = Var(model.time, model.AllTrays, model.Component, within = NonNegativeReals, initialize=40)
model.DtrayCompHoldupAmountDt = DerivativeVar(model.trayCompHoldupAmount)
model.EnergyHoldup = Var(model.time,model.AllTrays, initialize=-11000*40)
model.DEnergyHoldupDt = DerivativeVar(model.EnergyHoldup)
#-------------IRC变量定义---------------
...
def EnergyHoldupEqu(m,t,tray):
    return (m.EnergyHoldup[t,tray] - m.trayHoldupAmount[t,tray]*m.liquidLeavingMolarEnthalpy[t,tray])*1e-5 == 0
model.EnergyHoldupEqu = Constraint(model.time, model.AllTrays, rule=EnergyHoldupEqu)
...
#-------------------------------------------
# 对于本模型，正确的初始条件指定方式是：M(0),dM/dt(0),m1(0),dE/dt(0)
